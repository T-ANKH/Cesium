...
import buttonconfig from '../json/button-config.json';
...
export function createContentsButton(viewer) {
    ...
    // 解析JSON配置，生成目录项
    const items = Object.keys(buttonconfig);
    ...
    items.forEach(item => {
        ...
        // 设置点击事件
        const clickHandler = buttonconfig[item].onclick;
        const position = buttonconfig[item].position;
